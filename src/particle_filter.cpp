...
void ParticleFilter::init(double x, double y, double theta, double std[]) {
  ...
  // Initialize particle position (given estimates and GPS uncertainty)
  // Set all particle weights to 1
...
void ParticleFilter::dataAssociation(vector<LandmarkObs> predicted, 
  ...
  for (auto& obs : observations) {
    ...
    for (const auto& pred : predicted) {
      ...
      // If predicted measurement is closest to observed measurement;
      if (minD > distance) {
        // Assign observed measurement to landmark location
        minD = distance;
        obs.id = pred.id;
      }
    }
...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[], 
  ...
  /**
   * NOTE: You can read more about mult-variate Gaussian distribution here:
  ...
  for (auto& p : particles) {
    ...
    for(const auto& lm: map_landmarks.landmark_list) {
      ...
      // Save to predictions if the landmark is within the sensor range
    ...
    // NOTE: See equation this link: http://planning.cs.uiuc.edu/node99.html
